...
- (NSPersistentContainer *)persistentContainer {
    ...
    @synchronized (self) {
        if (_persistentContainer == nil) {
            _persistentContainer = [[NSPersistentContainer alloc] initWithName:@"GithubClient"];
    ...
    [self.managedObjectContext performBlockAndWait:^ {
        ...
        if ([self.managedObjectContext hasChanges]) {
            ...
            if (insertedObjects && insertedObjects.count > 0)
                [self.managedObjectContext obtainPermanentIDsForObjects:[insertedObjects allObjects] error:NULL];
            if ([self.managedObjectContext save:NULL])
                [self.managedObjectContext.parentContext performBlock:^{ [self.managedObjectContext.parentContext save:NULL]; }];
        }
    }];
}
...
- (NSURL *)applicationDocumentsDirectory {
    ...
    // code uses a directory named "ru.site.ChahainaMIX" in the application's
    // documents directory.
